...
async function employeePersona(employee: Employee, store: EvolutionStore, task = ''): Promise<PersonaBuild> {
  ...
  const text = [
    ...
    '6. 回复自然、简洁、直接；不要把私有记忆摘要原样复述，除非它与当前答案直接相关。',
    '',
    '【你的长期记忆摘要】',
    digest.text,
  ].join('\n')
...
export type StaffDispatchInput = {
  employeeId: string
  /** 给员工执行的完整工作上下文，由 Work Orchestrator 生成。 */
  text: string
  /** 履历里保存老板原始任务，不保存内部执行 Prompt。 */
  taskTitle?: string
...
export function apply(ctx: any, config?: any): OrgPanelCore | undefined {
  ...
  if (!tools || !subagents) {
    ctx?.logger?.warn?.('dsh-org-panel: employee runtime unavailable because tools/subagents is missing')
    return undefined
  }
  ...
  // 只有顶层 Work Orchestrator 显式绑定执行根。员工内部工具不允许把 child agent 覆盖成新的 root。
  ...
  const closeTask = async (taskId: string | null, employeeId: string, completion: { outcome: TaskOutcome; summary?: string; tools?: string[] }): Promise<void> => {
    ...
    try {
      await store.completeTask(taskId, {
        ...
        summary: completion.summary ? clip(completion.summary, 600) : undefined,
  ...
  const runEmployeeOnce = async (parent: any, employee: Employee, options: { prompt: string; personaTask: string; signal: AbortSignal; taskId?: string | null }): Promise<EmployeeRun> => {
    const provider = selectProvider(subagents)
    ...
    const run = await subagents.start(provider, {
      label: `赛博公司员工:${employee.id}:${employee.name}`,
    ...
    store.recordMemoryInjection({
      ...
      query: persona.query,
      memoryIds: persona.memoryIds,
      reflectionIds: persona.reflectionIds,
      taskId: options.taskId || undefined,
      childId: String(run.id ?? '') || undefined,
    })
    try {
      const result = await run.result
      return {
        runId: String(run.id ?? ''),
        reply: textOf(result?.output),
        stopReason: String(result?.stopReason ?? ''),
        isError: result?.isError === true,
        tools: toolsOfResult(result),
      }
    } finally {
      await run.dispose()
    }
  ...
  const memoryRecallTool = {
    name: MEMORY_TOOL,
    description: '读取某位数字员工自己的持久长期记忆。',
    parameters: { type: 'object', additionalProperties: false, required: ['staff', 'query'], properties: { staff: { type: 'string', enum: employees.map((item) => item.id) }, query: { type: 'string', minLength: 1 }, limit: { type: 'number', minimum: 1, maximum: 12 } } },
  ...
  const rememberTool = {
    name: REMEMBER_TOOL,
    description: '把稳定、可复用的信息写入某位员工的长期记忆。',
    parameters: { type: 'object', additionalProperties: false, required: ['staff', 'text'], properties: { staff: { type: 'string', enum: employees.map((item) => item.id) }, kind: { type: 'string', enum: ['preference', 'lesson', 'project', 'fact', 'relationship', 'workflow'] }, text: { type: 'string', minLength: 3 }, tags: { type: 'array', items: { type: 'string' }, maxItems: 12 }, importance: { type: 'number', minimum: 1, maximum: 5 } } },
  ...
  const skillLearnTool = {
    name: SKILL_TOOL,
    description: '把当前运行时真实存在的工具能力登记成某位员工的技能；等级只由真实执行证据决定。',
    parameters: { type: 'object', additionalProperties: false, required: ['staff', 'name'], properties: { staff: { type: 'string', enum: employees.map((item) => item.id) }, name: { type: 'string', minLength: 2 }, category: { type: 'string' }, summary: { type: 'string' }, source: { type: 'string', enum: ['experience', 'plugin', 'manual'] }, toolNames: { type: 'array', items: { type: 'string' }, minItems: 1, maxItems: 20 }, pluginNames: { type: 'array', items: { type: 'string' }, maxItems: 20 }, success: { type: 'boolean' } } },
    output: { schema: { type: 'object', additionalProperties: true }, render(_args: any, value: any) { return [{ type: 'text', text: `${value.staffName} 已登记技能：${value.skill.name} Lv.${value.skill.level}` }] } },
    isConcurrencySafe: () => true,
    async execute(args: any) {
      const employee = requireEmployee(args.staff)
      ...
      if (source === 'plugin' && !verified.length) {
        throw new Error(`插件类技能必须至少点名一个当前 Tool Registry 里真实存在的工具。${requested.length ? `你给的 ${requested.join('、')} 一个都不存在。` : '你一个工具都没给。'}先用 ${CAPABILITY_TOOL} 看看当前能力。`)
      }
      ...
      return { staffId: employee.id, staffName: employee.name, verifiedTools: verified, ignoredTools: requested.filter((name: string) => !verified.includes(name)), evidenceRecorded: false, ignoredSuccessClaim: typeof args.success === 'boolean' ? args.success : undefined, skill }
    },
  }

  const reflectTool = {
    name: REFLECT_TOOL,
    description: '员工完成任务后做复盘，沉淀可复用经验。',
    parameters: { type: 'object', additionalProperties: false, required: ['staff', 'task', 'outcome', 'lesson'], properties: { staff: { type: 'string', enum: employees.map((item) => item.id) }, task: { type: 'string', minLength: 2 }, outcome: { type: 'string', enum: ['success', 'partial', 'blocked', 'failed'] }, lesson: { type: 'string', minLength: 3 }, tags: { type: 'array', items: { type: 'string' }, maxItems: 12 } } },
  ...
  const snapshotTool = {
    name: SNAPSHOT_TOOL,
    description: '读取整家赛博公司的持久化快照，供空 Session 和诊断场景使用。',
    parameters: { type: 'object', additionalProperties: false, properties: { taskLimit: { type: 'number', minimum: 1, maximum: 50 }, memoryLimit: { type: 'number', minimum: 1, maximum: 30 } } },
    ...
    isConcurrencySafe: () => true,
    async execute(args: any) { return snapshot({ taskLimit: Number(args?.taskLimit), memoryLimit: Number(args?.memoryLimit) }) },
  }

  const capabilityScanTool = {
    name: CAPABILITY_TOOL,
    description: '扫描当前 DSH 运行时真实暴露的工具能力。',
    parameters: { type: 'object', additionalProperties: false, properties: { staff: { type: 'string', enum: employees.map((item) => item.id) }, query: { type: 'string' } } },
    output: { schema: { type: 'object', additionalProperties: true }, render(_args: any, value: any) { return [{ type: 'text', text: JSON.stringify(value, null, 2) }] } },
    isConcurrencySafe: () => true,
    async execute(args: any) {
      ...
      return { staffId: args.staff ? String(args.staff) : undefined, count: visible.length, groups: grouped, note: '这里只列出当前运行时能发现的真实工具。' }
    },
  }
  ...
  const dispatch = async (input: StaffDispatchInput): Promise<StaffDispatchOutcome> => {
    ...
    if (!text) return fail('工作上下文为空，没有可执行的任务。')
    const parent = resolveAgent(input.agent)
    if (!parent) return fail('赛博公司还没有可用的 DSH 执行根。请先从工作台发起一次真实 company_work，再让外部渠道继续工作。')

    const signal = input.signal || new AbortController().signal
    const taskId = await openTask(employee, input)
    let run: EmployeeRun
    try {
      run = await runEmployeeOnce(parent, employee, { prompt: text, personaTask: input.taskTitle || input.taskDescription || text, signal, taskId })
    } catch (error) {
    ...
    await closeTask(taskId, employee.id, {
      outcome,
      summary: violations.length ? `只读来源观测到写工具：${violations.join('、')}` : (run.reply || `子代理结束原因：${run.stopReason || '未知'}`),
      tools: run.tools,
    })

    if (violations.length) return { ok: false, employeeId: employee.id, employeeName: employee.name, reply: run.reply, error: `只读来源观测到写工具：${violations.join('、')}`, tools: run.tools, outcome, taskId: taskId || undefined }
    if (outcome === 'failed' || outcome === 'blocked') return { ok: false, employeeId: employee.id, employeeName: employee.name, reply: run.reply, error: `员工运行异常结束：${run.stopReason || outcome}`, tools: run.tools, outcome, taskId: taskId || undefined }
