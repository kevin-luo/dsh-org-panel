...
// 密钥只以 env:/secret: 引用形式配置，解析出的明文只在 Adapter 实例内存里存在。
...
import {
  allowsWrite, maskSecretValue, normalizeCommunicationConfig,
...
export class CommunicationManager {
  ...
  constructor(private readonly deps: CommunicationManagerDeps) {
    ...
    this.router = new WorkRouter({
      gateway: this.gateway,
      roster: deps.roster,
      bindings: deps.config.channelBindings,
  ...
  /** 对外摘要只含掩码与事实状态。maxEmployeeHops 字段暂保留 API 形状，值固定为工作组硬上限 4。 */
  async summary(): Promise<CommunicationSummary> {
    ...
    return { configured: this.configured, adapters, channelBindings: this.deps.config.channelBindings, maxEmployeeHops: 4 }
  }
    ...
    emit(event) {
      ...
      if (event.type === 'external.message.received') {
        bus.publish({
          ...
          platform: event.platform, conversationId: event.conversationId, preview: clip(event.text), senderName: event.senderName,
        }, event.platform)
        return
      }
...
export function registerCommunication(
  ...
  if (tools?.register) {
    ...
    tools.register({
      ...
      parameters: {
        ...
        properties: {
          adapterId: { type: 'string' }, conversationId: { type: 'string' }, text: { type: 'string', minLength: 1 },
          staff: { type: 'string', description: '真实 employeeId。' },
        },
      },
  ...
  if (manager.configured) {
    ctx?.systemPrompt?.section?.({
      ...
      text: [
        '【赛博公司 · 外部通讯制度】',
        '飞书、QQ、微信的消息统一经过 IM Gateway 鉴权，再进入 Work Router，最终交给同一个 Work Orchestrator 自动组队。',
        '渠道层不选择主 Agent、不默认交给秘书，也不维护员工转交链。每位员工在所有渠道共享同一份长期记忆、技能、插件绑定、模型能力和履历。',
        '外部消息自带 actorRole 与 permissionMode。Read Only 来源若真实观测到写工具，员工回复会被拦下并留下越权事件。',
        '需要查看渠道状态用 company_comm_status；主动外发用 company_comm_send，只能发往已配置且可写的会话。',
      ].join('\n'),
    })
  }
